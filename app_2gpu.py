...
# load diffusion model on GPU 0
print('Loading diffusion model on GPU 0...')
...
# load reconstruction model on GPU 1
print('Loading reconstruction model on GPU 1...')
...
def preprocess(input_image, do_remove_background):
    ...
    if do_remove_background:
        input_image = remove_background(input_image, rembg_session)
        input_image = resize_foreground(input_image, 1.0625)  # Increase scaling factor
...
def make3d(images):
    ...
    with torch.no_grad():
        # get triplane on GPU 1
        planes = model.forward_planes(images, input_cameras)

        # get video on GPU 1
        chunk_size = 20 if IS_FLEXICUBES else 1
        render_size = 480  # Increase render size
